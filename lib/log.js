...
function format(body) {
  ...
  var timezone = config.app && config.app.timezone ? config.app.timezone : "UTC"
    , datetime = moment(body.sec).tz(timezone).format("YYYY-MM-DD HH:mm:ss.SSS")
    , level    = ["ERROR", "INFO", "WARN", "DEBUG"][body.level]
    , code     = util.format("%s:%s", body.file, body.line);

  if (process.env.LOCAL) {
    return util.format("[%s] [A] [%s] %s %s %s", datetime, level, body.message, body.user, code);
  } else {
