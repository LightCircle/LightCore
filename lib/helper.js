 ...
 * @returns {*|Array}
 */
exports.tree = function (folder, result, filter) {
  ...
  _.each(files, function (file) {
    ...
    if (stats.isDirectory()) {
      return exports.tree(full, result, filter);
    }
...
exports.decodeToken = function(req, secret) {
  ...
  var body = (req.body && req.body.access_token)
    , query = (req.query && req.query.access_token)
    , header = req.headers["x-access-token"];

  if (body || query || header) {
    return jwt.decode(body || query || header, secret) || {};
  }
