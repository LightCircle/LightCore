...
exports.dateFormat = function (data, option, handler) {
  if (!data) {
    return data;
  }
  ...
  // 没有指定格式
  if (!option) {
    return moment(data).tz(handler.timezone).format('YYYY-MM-DD');
  }

  // option为字符串
  if (typeof option === 'string') {
    return moment(data).tz(handler.timezone).format(option);
  }

  // option为对象
  option = option || {};
  return moment(data).tz(option.timezone || handler.timezone).format(option.format || 'YYYY-MM-DD');
};
