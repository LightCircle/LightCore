...
exports.unique = function (handler, rule, callback) {
  ...
  var db = handler.db || {}
    , model = new Model(handler.domain, handler.code, rule.option[0], undefined, db.user, db.pass)
...
exports.exists = function (handler, rule, callback) {
  ...
  var db = handler.db || {}
    , model = new Model(handler.domain, handler.code, rule.option.table, undefined, db.user, db.pass)
    , values = mpath.get(rule.key, handler.params)
    , errors = [];

  if (_.isUndefined(values)) {
    return callback(error, {name: rule.name, message: rule.message});
  }
  ...
  async.eachSeries(values, function (value, next) {
    ...
    _.each(rule.option.conditions, function (val, key) {
      if (!(_.isString(val) && val.startsWith("$"))) {
        return;
      }

      if (val.substr(1) == rule.key) {
        rule.option.conditions[key] = value;
      } else {
        var ref = mpath.get(val.substr(1), handler.params);
        rule.option.conditions[key] = {$in: _.isArray(ref) ? ref : [ref]};
      }
    });

    // 获取件数
    model.count(_.extend({valid: 1}, rule.option.conditions), function (err, result) {
      if (err) {
        return next(err);
      }
      ...
      var original = handler.params.data._original[rule.key.substr(5)] || value;
      errors.push({name: rule.name, message: rule.message, value: original});
      next(error);
    });
  }, function (err) {
    callback(err, errors);
