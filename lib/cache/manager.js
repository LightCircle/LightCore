...
exports.init = function (domain, collection, callback) {
  ...
  functions.configuration = function (done) {
    var select = "type,key,value,valueType,displayType,description"
  ...
  functions.validator = function (done) {
    var select = "group,name,rule,key,option,message,sanitize,class,action,condition,strict,description"
  ...
  functions.structure = function (done) {
    var select = "public,lock,type,kind,tenant,version,schema,items,extend,tenant,description,parent"
  ...
  functions.board = function (done) {
    var select = "schema,api,type,kind,path,class,action,filters,selects,sorts,reserved,script,description"
  ...
  functions.route = function (done) {
    var select = "template,url,class,action,description"
  ...
  functions.tenant = function (done) {
    var select = "code,name,logo,description"
      , model  = new Model(domain, constant.SYSTEM_DB_PREFIX, constant.SYSTEM_DB_TENANT);

    model.getBy({valid: 1}, 0, Number.MAX_VALUE, undefined, select, function (err, result) {

      db = model.db;
      log.debug("Loading tenant : " + result.length);
      cache.set(constant.SYSTEM_DB_TENANT, result);
      done(err);
    });
  };
  ...
  // 加载所有表
  return async.series([
    functions.configuration, functions.validator, functions.i18n, functions.structure, functions.board, functions.route, functions.tenant
  ], function (err) {
    callback(err, db);
  });
};
...
function loadCacheFromData(callback) {
  ...
  // 初始化config与i18n
  config.load(cache.get(constant.SYSTEM_DB_CONFIG));
  i18n.load(cache.get(constant.SYSTEM_DB_I18N));
