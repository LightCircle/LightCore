...
function loadFromFile(callback) {
  ...
  const structure  = loadYaml(CONST.SYSTEM_DB_STRUCTURE)
    , sysStructure = loadYaml(CONST.SYSTEM_DB_STRUCTURE + '.sys');
  cache.set(CONST.SYSTEM_DB_STRUCTURE, structure.concat(sysStructure));

  const board  = loadYaml(CONST.SYSTEM_DB_BOARD)
    , sysBoard = loadYaml(CONST.SYSTEM_DB_BOARD + '.sys');
  cache.set(CONST.SYSTEM_DB_BOARD, board.concat(sysBoard));
  ...
  log.debug(`Loading structure : ${structure.length + sysStructure.length}`);
  log.debug(`Loading board : ${board.length + sysBoard.length}`);
