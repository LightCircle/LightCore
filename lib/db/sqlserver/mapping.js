  ...
  rsParse(data, callback) {
    ...
    async.map(data, (columns, done) => {
      ...
      async.reduce(columns, {}, (memo, item, next) => {
        ...
        // XML类型的值，转换成JSON
        if (item.type === 'XML') {
          return this._unWrapXML(item.value, (err, json) => {
            memo[item.name] = json;
            next(null, memo);
          });
        }
  ...
  buildXML(data) {
    ...
    Object.keys(data).forEach(key => {
      ...
      if (Array.isArray(value)) {
        return data[key] = this._wrapXML(Mapping._listToElement(value));
      }
