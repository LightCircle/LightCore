...
class Controller {
  ...
  constructor(handler, params) {
    ...
    this.params = params ? params : handler.params;
    this.params.data = this.params.data || {};
    this.params.condition = this.params.condition || {};

    this.model = new Model(handler.domain, handler.code);
  }

  list(callback) {

    this._setValid();
    this.model.list(this.params.script, this.params.condition, (err, result) => {
      if (err) {
        return callback(err);
      }

      if (this.params.limit > 0) {
        return this._countList((err, count) => {
          if (err) {
            return callback(err);
          }
  ...
  remove(callback) {
    ...
    this.model.remove(this.params.script, this.params.condition, callback);
  }
  ...
  update(callback) {
    ...
    const condition = this.params.id ? {_id: this.params.id} : this.params.condition;

    this._setValid();
    this.model.update(this.params.script, this.params.data, condition, callback);
  }
  ...
  get(callback) {
    ...
    this._setValid();
    this.model.get(this.params.script, condition, callback);
  }
