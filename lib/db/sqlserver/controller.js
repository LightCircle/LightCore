...
class Controller {
  ...
  constructor(handler, table) {
    ...
    // controller和model会对params进行修改，所以在这儿clone一份
    this.params = _.cloneDeep(handler.params);
    this.params.data = _.cloneDeep(this.params.data || {});
  ...
  list(callback) {
    ...
    // list方法内部对condition做了修改，为了不影响下面的count方法，在这儿clone一份condition
    this.model.list(this.params.script, _.cloneDeep(this.params.condition), (err, result) => {
      ...
      if (this.params.limit > 0) {
        return this._countList(_.cloneDeep(this.params.condition), (err, count) => {
          if (err) {
            return callback(err);
          }
  ...
  _countList(condition, callback) {
    ...
    sql = sql.replace(/[ ]+OFFSET[ ]+\d+[ ]+ROW$/, '');
    sql = sql.replace(/[ ]+ORDER BY.*(ASC|DESC)$/, '');
