...
class Operator {
    ...
    let and = Object.keys(params).map(key => {
      ...
      if (typeof value === 'object') {
        return Object.keys(value).map(k => {
          ...
          // 如果值是数组，并且数组值为空，那么忽略这个条件， {_id: {$in: []}} => [id] IN (null)
          ...
          if (mapping.isHash(v)) {
            ...
            _.set(out.condition, `${key}${k}`, v); // 把值保存到condition中
          }
          ...
          // 是操作符号，直接进行比较
          if (k.startsWith('$')) {
            return Operator.getCompiler({key: key, operator: k, parameter: `${key}${k}`}, schema, out);
          }
  ...
  static _getXmlCompiler(define, compiler, params) {
    ...
    const key = compiler.parameter
      , val   = helper.get(params.condition, key);
  ...
  static _getXmlColumnDefine(schema, column) {
    ...
    const struct = Operator.structure(schema);
    if (!struct) {
      return null;
    }
  ...
  static structure(schema) {
    ...
    const list = cache.get(CONST.SYSTEM_DB_STRUCTURE);
    if (!list) {
      return null;
    }
    return list.find(item => item.schema === schema);
  }
}
