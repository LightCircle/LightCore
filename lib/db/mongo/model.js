...
class Model {
  ...
  parseCode(table, code) {
    ...
    // 表名复数化
    const pluralize = helper.collection(table);
    ...
    if (this.isSystemTable(table)) {
      return `${constant.SYSTEM_DB_PREFIX}.${pluralize}`;
    }

    return code ? `${code}.${pluralize}` : pluralize;
  }
  ...
  isSystemTable(table) {
    ...
    let systemTalbe = constant.SYSTEM_TABLE;
    if (config.app && config.app.systemTable) {
      systemTalbe = systemTalbe.concat(config.app.systemTable);
    }

    return systemTalbe.findIndex(item => item === table) >= 0;
  }
  ...
  add(insert, callback) {
    ...
    db(this, (err, db) => {
      db.insert(insert, (err, result) => {
        ...
        log.info(`Collection  : ${this.domain} # ${this.code} # add`);
  ...
  remove(id, update, callback) {
    ...
    db(this, (err, db) => {
      ...
      log.info(`Collection  : ${this.domain} # ${this.code} # remove`);
      log.info(`Condition   : ${JSON.stringify(filter)}`);
  ...
  update(id, update, callback) {
    ...
    db(this, (err, db) => {
      ...
      log.info(`Collection  : ${this.domain} # ${this.code} # update`);
      log.info(`Condition   : ${JSON.stringify(filter)}`);
      log.info(`Data        : ${JSON.stringify(update)}`);
    ...
    db(this, (err, db) => {
      ...
      db.updateMany(filter, update, (err, result) => {
        ...
        if (result.matchedCount === 1) {
          return db.findOne(filter, callback);
        }
  ...
  get(filter, select, callback) {
    ...
    db(this, (err, db) => {
      ...
      log.info(`Collection  : ${this.domain} # ${this.code} # get`);
      log.info(`Condition   : ${JSON.stringify(filter)}`);
      log.info(`Option      : ${JSON.stringify(options)}`);
  ...
  getBy(filter, skip, limit, sort, select, callback) {
    ...
    db(this, (err, db) => {
      ...
      log.info(`Collection  : ${this.domain} # ${this.code} # list`);
      log.info(`Condition   : ${JSON.stringify(filter)}`);
      log.info(`Option      : ${JSON.stringify(options)}`);
  ...
  total(filter, callback) {
    ...
    db(this, (err, db) => {
      ...
      log.info(`Collection  : ${this.domain} # ${this.code} # total`);
      log.info(`Condition   : ${JSON.stringify(filter)}`);
  ...
  dropUser(user, callback) {
    ...
    db(this, err => {
      ...
      this.db.removeUser(user, callback);
    });
  }
