...
exports.send = function (server, port, key, data, callback) {
  ...
  server = process.env.DEV ? "127.0.0.1" : server;
  port = process.env.DEV ? (process.env.PORT || 7000) : (port ||config.app.port);

  client.connect(util.format("ws://%s:%s%s", server, port, exports.DOMAIN), "echo-protocol");
};
