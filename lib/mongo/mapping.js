 ...
 * @module light.framework.mongo.mapping
 * @version 1.0.0
 */
"use strict";
 ...
 * @param define
 */
exports.default = function (object, define) {
  if (!define || !object) {
    return object;
  }
  ...
  if (_.isArray(object)) {
    _.each(object, function (item) {
      setDefault(item, define);
    });
  } else {
    setDefault(object, define);
  }
 ...
 * @param define
 */
function setDefault(object, define) {
  ...
  _.each(_.keys(define), function (key) {
    ...
    if (!_.isUndefined(define[key].default) && _.isUndefined(object[key])) {
      ...
      if (needParse) {
        object[key] = type.dataParse(object[key], define[key]);
      }
    }
 ...
 * @param define
 */
exports.dataParseAll = function (object, define) {
  ...
  if (_.isArray(object)) {
    _.each(object, function (item) {
      dataParse(item, define);
    });
  } else {
    dataParse(object, define);
  }

  return object;
};

function dataParse(object, define) {

  _.each(_.keys(object), function (key) {
    // 利用操作符对应的方法转换，如果没有找到操作符的定义则不做转换
    var hasMongoOperator = (/^\$.*/i).test(key);
    if (hasMongoOperator) {
      object[key] = operator.parse(key, object[key], define);
      return;
    }
    ...
    if (_.isObject(object[key]) && containsMongoOperator) {
      return exports.dataParseAll(object[key], option);
    }

    // 如果option.contents有值且不是基础类型，则递归解析
    if (!_.isUndefined(option.contents) && !_.contains(type.dataTypes, typeLowerCase(option.contents))) {
      object[key] = exports.dataParseAll(object[key], option.contents);
    }
    // 否则直接转换类型
    else {
      object[key] = type.dataParse(object[key], option);
    }
  });
}
 ...
 * @param define
 */
exports.queryParseAll = function (object, define) {
  ...
  if (_.isArray(object)) {
    _.each(object, function (item) {
      queryParse(item, define);
    });
  } else {
    queryParse(object, define);
  }

  return object;
};

function queryParse(object, define) {
  ...
  _.each(_.keys(object), function (key) {
    ...
    if (_.isObject(object[key]) && containsMongoOperator) {
      return exports.queryParseAll(object[key], option);
    }

    // 如果option.contents有值且不是基础类型，则递归解析
    if (!_.isUndefined(option.contents) && !_.contains(type.dataTypes, typeLowerCase(option.contents))) {
      object[key] = exports.queryParseAll(object[key], option.contents);
    }
    // 否则直接转换类型
    else {
      object[key] = type.queryParse(object[key], option);
    }
  });
}
