 ...
 * @param {Object} options.define collection定义
 * @param {String} options.user 数据库用户名
 * @param {String} options.pass 数据库密码
 * @param {String} options.tz 时区信息
 */
var Model = module.exports = function Model(domain, code, table, options) {
  ...
  } else if (_.contains(constant.SYSTEM_TABLE, table)) {
    this.code = constant.SYSTEM_DB_PREFIX + "." + this.table;
  } else {
 ...
 *  contentType
 */
Model.prototype.fileToGrid = function (file, callback) {
  ...
  griddb(this, function (err, db) {
    ...
    store.open(function (err, grid) {
      ...
      var filePath = fs.realpathSync(path.join(file.base, path.basename(file.path)));
...
Model.prototype.streamToGrid = function (file, callback) {
  griddb(this, function (err, db) {
 ...
 * @param callback
 */
Model.prototype.readStreamFromGridInRange = function (file, start, end, callback) {
 ...
 * @param callback
 */
Model.prototype.readStreamFromGrid = function (file, callback) {
 ...
 * @param callback
 */
Model.prototype.dropCollection = function (callback) {
  db(this, function (err) {
    if (err) {
      return callback(err);
    }
 ...
 * @param callback
 */
Model.prototype.command = function (command, option, admin, callback) {
 ...
 * @param callback
 */
Model.prototype.dropSession = function (id, callback) {
 ...
 * @param {Function} callback
 */
Model.prototype.serverStatus = function (admin, callback) {
 ...
 * @param {Function} callback
 */
Model.prototype.dbStatus = function (dbname, callback) {
