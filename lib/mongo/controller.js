 ...
 * @module light.framework.mongo.controller
 * @version 1.0.0
 */
...
var Controller = module.exports = function Controller(handler, table, define) {
  ...
  var domain  = handler.domain || constant.SYSTEM_DB
    , code    = handler.code
    , account = handler.db || {}
...
Controller.prototype.dropUser = function (callback) {
  ...
  this.model.dropUser(this.data.user, function (err, result) {console.log(err, result);
    if (err) {
      return callback(new errors.db.Remove());
    }
