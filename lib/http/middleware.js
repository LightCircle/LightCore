...
exports.lang = function (req, res, next) {
  ...
  // cookie的语言优先
  var language = req.cookies["light.lang"];
...
exports.permission = function (req, res, next) {
  ...
  var url    = req.url
    ...
    , targetAccess = _.find(req.session.access,function(item){return Object.keys(item)[0] === req.session.domain})
    , access = targetAccess && !_.isEmpty(targetAccess) ? targetAccess[req.session.domain] : {}
    , accessUrlList = _.filter(access.items, function(item){return item.type === "route" || item.type === "board"; });

  // 包含该url的access列表
  var accessTargetItemList = _.filter(accessUrlList, function (item) {
    return _.contains(item.resource, url);
  });
  var accessTargetAuthList = _.union(_.pluck(accessTargetItemList, "authes"))[0];

  if (!_.isEmpty(accessTargetItemList) && _.isEmpty(_.intersection(authority, accessTargetAuthList))) {
    log.error("Access deny");
    return res.redirect(config.app.error);
  } else {
    next();
  }
};
