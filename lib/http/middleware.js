...
function getCookie(req, key) {console.log(req.headers.cookie);
...
exports.permission = function (req, res, next) {
  ...
  var code = req.session.code ? req.session.code : constant.DEFAULT_TENANT;
  if (req.session.access === undefined || !req.session.access[code]) {
    rider.access.list(new context().bind(req, res), function (err, accesses) {
      if (err) {
        log.error(err);
        throw err;
      } else {
        req.session.access || (req.session.access = {});
        req.session.access[code] = accesses.items;
...
function checkAccess (req, res, next) {
  var url    = req.url
    ...
    , code = req.session.code ? req.session.code : constant.DEFAULT_TENANT
    , access = req.session.access[code];
  ...
  var accessTargetAuthList = _.flatten(_.union(_.pluck(accessTargetItemList, "authes")));
