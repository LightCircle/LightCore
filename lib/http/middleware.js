...
exports.lang = function (req, res, next) {
  ...
  // cookie的语言优先
  var language = helper.getCookie(req, "light.lang");
  ...
  if (!language) {
    ...
    if (clientLangs) {
      var langs = _.compact(acceptLanguage.parse(clientLangs));
      _.each(langs, function (item) {
        if (_.contains(['zh', 'ja', 'en'], item.language)) {
...
exports.permission = function (req, res, next) {
  ...
  var handler = new context().bind(req, res)
    ...
    ;
  handler.code = handler.code || constant.DEFAULT_TENANT;
