...
exports.send = function(res, error, data) {
  ...
  if (res.statusCode == 408) {
    return log.error(res.statusMessage);
  }
  ...
  if (error) {
    ...
    return res.status(code).send(createErrorSchema(error.code, error.message, data));
  }
...
exports.sendError = function(res, error) {
  ...
  // 返回错误信息
  return res.status(error.code).send(createErrorSchema(error.code, error.message, error.errors));
};
