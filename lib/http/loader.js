...
const path         = require("path")
  ...
  , TediousStore   = require('connect-tedious')(session)
  ...
  , mysql          = require("../db/mysql/connection")
  , sqlserver      = require("../db/sqlserver/connection")
  , tool           = require("../tool")
  ;
...
function setupExpress(app, db) {
  ...
  morgan.token('date', function getId() {
    // 这里不设置timezone，个log函数一样使用系统的时区（0时区）
  ...
  app.use(function(req, res, next) {
    try {
      const sid = helper.decodeToken(req, config.app.tokenSecret).sid;
      if (sid) {
...
function getSessionStore(db) {

  // sqlserver
  if (process.env.LIGHTSQLSERVER_HOST) {
    const setting = {
      config: Object.assign({}, sqlserver.db().connectionConfig)
    };

    setting.config.options.tableName = '[dbo].[sessions]';
    return new TediousStore(setting);
  }

  // mysql
  if (process.env.LIGHTMYSQL_HOST) {
    return new MySQLStore(mysql.db().config.connectionConfig);
  }

  // mongodb
  return new MongoStore({db: db});
}
...
exports.loadConfig = function () {
  ...
  // 加载配置文件
  const setting = helper.yamlLoader('/config.yml');
  ...
  if (setting.mysql) {
    process.env.LIGHTMYSQL_HOST = setting.mysql.host;
    process.env.LIGHTMYSQL_PORT = setting.mysql.port;
    process.env.LIGHTMYSQL_USER = setting.mysql.user;
    process.env.LIGHTMYSQL_PASS = setting.mysql.pass;
  }

  if (setting.sqlserver) {
    process.env.LIGHTSQLSERVER_HOST = setting.sqlserver.host;
    process.env.LIGHTSQLSERVER_PORT = setting.sqlserver.port;
    process.env.LIGHTSQLSERVER_USER = setting.sqlserver.user;
    process.env.LIGHTSQLSERVER_PASS = setting.sqlserver.pass;
  }
};
...
exports.parseArgv = function () {
  ...
  const isPush  = (process.argv.length > 3 && process.argv[3] === '-push')
...
exports.preLoadController = function (ignore) {
  ...
  _.each(ctrl, function (item) {
    const source = item.file.replace(process.cwd, ".");
    source.replace(".js", "");
    require(source);
  });
};
