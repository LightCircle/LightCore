...
exports.menu = function (handler, callback) {
  ...
  async.series([
    ...
    // access
    function (done) {
      access = _.find(handler.req.session.access,function (item) {return Object.keys(item)[0] === domain})[domain];
    ...
    if(handler.code == 'light'){
      menus = _.reject(menus,function(item){
        return _.contains(['permission','datacenter'],item.menu);
      });
    }
...
exports.code = function () {
  return {
    ...
    system: {
      source: _.union(
        resolve(__model + "/lib"),
        resolve(__bridge + "/lib"),
        resolve(__framework + "/lib")
      ),
      unit: _.union(
        resolve(__model + "/test"),
        resolve(__bridge + "/test"),
        resolve(__framework + "/test")
      )
    }
  }
};
