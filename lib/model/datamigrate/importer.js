...
ETL.prototype.initialize = function (callback) {
  ...
  async.series([
    ...
    init(this.controller, this.handler, this.primitive, callback);
  }.bind(this));
...
ETL.prototype.extract = function (callback) {
  ...
  if (this.type == 'excel') {
    ...
    return async.eachSeries(data, function (sheet, next) {
      ...
      // 尝试调用用户自定义的 befor 方法
      before(self.controller, self.handler, self.primitive, callback);
    });
  }
...
ETL.prototype.transform = function (callback) {
  ...
  self.primitive.getCursor({filter: _.extend({valid: 1}, this.condition)}, function (err, curosr) {
...
ETL.prototype.add = function (row, callback) {
  ...
  if (this.allowUpdate) {
    ...
    // 判断有, 则更新, 没有就插入一条新数据
    this.handler.params.condition = condition;
...
ETL.prototype.load = function (callback) {
  ...
  // 尝试调用用户的方法
  after(this.controller, this.handler, this.processed, function (err) {
    if (err) {
      return callback(err);
    }

    var self = this, more = true;
    this.processed.getCursor({filter: {valid: 1}}, function (err, curosr) {
      async.whilst(
        function () {
          return more;
        },
...
function init(controller, handler, model, callback) {
...
function before(controller, handler, model, callback) {
...
function after(controller, handler, model, callback) {
  if (!controller) {
    return callback(undefined);
  }
