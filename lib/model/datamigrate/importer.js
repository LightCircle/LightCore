...
ETL.prototype.extract = function (callback) {
  ...
  if (this.type === 'excel') {
    ...
    let sheets = excel.parse(this.handler.params.files[0].path, this.mapping)
      , data = [];
    ...
    // return async.eachSeries(sheets, function (sheet, next) {
      ...
      function (loop) {
        ...
        async.waterfall([
          ...
          function (hasError, parsed, next) {
            ...
            if (hasError) {
              if (self.allowError && (self.allowErrorMax > 0 && self.log.length < self.allowErrorMax)) {
                return next();
              }
...
ETL.prototype.parse = function (row, callback) {
  ...
  async.waterfall([
    ...
    // 类型转换
    function (next) {
      // const keys = self.mapping.map(item => item.variable || item.key);
      async.eachSeries(self.mapping, (item, loop) => {

        const key = item.variable || item.key, val = row[key];
    ...
    function (next) {
      self.handler.params.data = row;
      validator.isValid(self.handler, self.validator, function (err, message) {
        if (err) {
          hasError = true;
