        ...
        async.waterfall([
          ...
          function (row, next) {
            ...
            validator.isValid(self.handler, self.check, function (err, message) {
              ...
              // 将数据插入到, 存放加工完数据的表中
              delete row['_id'];
              self.processed.add(getInsertData(row, self.uid), loop);
            });
          },

          // 尝试调用开发者自定义的 ctrl 对行数据进行加工
          function (row, next) {
            parse(self.controller, self.primitive, row, next);
          },
...
new ETL(
  {domain: 'LightDB', table: 'excel', user: 'light', pass: '2e35501c2b7e'},
  {
    ...
    file: {
      type: 'excel',
      name: '/Users/lilin/developer/LightApps/SmartKeyRepair/template/staff.xlsx',
      mapping: {
        '1': 'id', // "ID"
        '2': 'name', // "姓名"
        '3': 'number', // "工号"
