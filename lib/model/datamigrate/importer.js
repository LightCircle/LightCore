...
var ETL = module.exports = function ETL(handler, options) {
  ...
  this.primitive = new Model(this.handler.domain, undefined, table, undefined, db.user, db.pass);

  // 存放加工好的数据collection
  table = options.processed || PREFIX + helper.randomGUID4();
  this.processed = new Model(this.handler.domain, undefined, table, undefined, db.user, db.pass);
...
ETL.prototype.extract = function (callback) {
  ...
  if (this.type == 'mongodb') {
    var source = this.name, more = true
      ...
      , model = new Model(source.domain, source.code, source.table, undefined, source.user, source.pass);
...
ETL.prototype.add = function (row, callback) {
  ...
  if (this.type == 'mongodb') {
    var db = this.handler.db || {}
      , model = new Model(this.handler.domain, undefined, this.talbe, undefined, db.user, db.pass);
