  ...
  async.waterfall([
    ...
    function (next) {
      _.each(row, function (val, key) {
        var item = _.findWhere(_.values(self.mapping), {key: key});
        if (item && item.type && !_.isUndefined(val)) {
          if (item.type.toLocaleLowerCase() == 'array') { // 数组类型
            row[key] = val.split(/[ ]*,[ ]*/)
          }
        }
      });
    ...
    function (next) {
      ...
      validator.isValid(self.handler, self.check, function (err, message) {
    ...
    // 尝试调用开发者自定义的数据校验
    function (next) {
      // TODO: 维修类型2 需要检查 指定的维修类型1的子类型里存在
      valid(self.controller, self.handler, row, function (err, result) {
    ...
    delete row['_id'];
    callback(err, getInsertData(row, self.uid));
  });
};
...
new ETL(handler, doc).exec('/Users/lilin/developer/LightApps/SmartKeyRepair/template/staff.xlsx', function () {
  process.exit(0);
});
