...
// 6. 添加或更新
// 9. staff id uniq 的支持
// 10. 加工好的数据临时保存起来, 等全部OK了, 再导入到真正的表里
...
ETL.prototype.add = function (row, callback) {
  ...
  if (this.allowUpdate) {
    ...
    this.handler.params.condition = _.reduce(this.uniqueKey, function (memo, key) {
      memo[key] = row[key];
      return memo;
    }, {});

    // 判断有, 则更新, 没有就插入一条新数据
    this.processed.get(this.handler, function (err, result) {
      if (err) {
        return callback(err);
      }

      if (result) {
        this.handler.params.data = {$set: parsed};
        this.processed.update(this.handler, next);
      } else {
        this.handler.params.data = parsed;
        this.processed.add(this.handler, next)
      }
    }.bind(this));
  } else {
    this.handler.params.data = parsed;
