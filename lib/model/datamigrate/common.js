...
exports.flatten = function (handler, mapping) {
  ...
  let data = handler.params.data, values = helper.get(data, key);
...
exports.getLinkData = function (handler, mapping, callback) {
  ...
  let values = helper.get(data, mapping.key)
  ...
  async.mapSeries(isArray ? values : [values], (value, loop) => {
    ...
    _.each(condition, (val, key) => {
      ...
      if (val.substr(1) === mapping.key) {
        ...
        val = helper.get(data, val.substr(1));
    ...
    if (isArray) {
      helper.set(data, index, _.map(result, item => {
        data._original[item.original] = item.key;
        return item.val;
      }));
    } else {
      helper.set(data, index, result[0].val);
...
exports.end = function (controller, handler, result, callback) {

  if (!controller) {
    return callback();
  }

  const func = controller['end'];
  if (!func) {
    return callback();
  }

  func.call(this, handler, result, callback);
};
