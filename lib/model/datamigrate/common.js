...
exports.flatten = function (handler, mapping) {
  ...
  let data = handler.params.data, values = mpath.get(key, data);
...
exports.getLinkData = function (handler, mapping, callback) {
  ...
  let values = mpath.get(mapping.key, data)
  ...
  async.mapSeries(isArray ? values : [values], (value, loop) => {
    ...
    if (isArray) {
      mpath.set(index, data, _.map(result, item => {
        data._original[item.original] = item.key;
        return item.val;
      }));
    } else {
      mpath.set(index, data, result[0].val);
