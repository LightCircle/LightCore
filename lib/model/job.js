...
exports.init = function (domain, callback) {
  ...
  var handler = new context().create(constant.ADMIN_ID, domain)
    , model = new Model(domain, constant.SYSTEM_DB_PREFIX, CONST.SYSTEM_DB_JOB, {tz: handler.timezone})
    , condition = {valid: 1, run: constant.JOB_STARTED};

  // 获取任务一览
  model.getBy(condition, 0, Number.MAX_VALUE, function (err, result) {
    _.each(result, function (item) {
      if (helper.isBlank(item.schedule)) {
        return callback();
      }
...
exports.immediate = function (handler, callback) {
  ...
  rider.job.get(handler, (err, item) => {

    job.immediate(item, (err, result) => {
...
function updateJob(handler, id, error, result, callback) {
  ...
  // 更新
  const model = new Model(handler.domain, constant.SYSTEM_DB_PREFIX, CONST.SYSTEM_DB_JOB, {tz: handler.timezone});
