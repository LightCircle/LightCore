...
signal.addListener("start.job", function (handler, callback) {

  if (isMaster()) {
    return exports.start(handler, callback);
  }
...
signal.addListener("stop.job", function (handler, callback) {

  if (isMaster()) {
    return exports.stop(handler, callback);
  }
...
/**
 * @ignore
 * @desc 判断当前进程是否为主服务。<br>通过查看命令行参数里是否有[ master ]来判断
 * @returns {Boolean} true : 是主服务, false: 非主服务
 */
function isMaster() {
  return _.contains(_.last(process.argv, process.argv.length - 2), "master");
}
...
exports.init = function (domain) {
  ...
  var handler = new context().create(constant.ADMIN_ID, domain)
    , model = new Model(domain, constant.SYSTEM_DB_PREFIX, constant.MODULES_NAME_JOB)
...
function updateJob(handler, id, error, result, callback) {
  ...
  // 更新
  var model = new Model(handler.domain, constant.SYSTEM_DB_PREFIX, constant.MODULES_NAME_JOB);
