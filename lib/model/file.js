...
exports.image = function (handler) {
  ...
  rider.file.get(handler, function (err, result) {
    ...
    new Ctrl(handler).readFromGrid(function (err, data) {
      if (err || !result) {
        return res.status(404).end();
      }
...
exports.download = function (handler) {
  ...
  var req = handler.req, res = handler.res;
  ...
  rider.file.get(handler, function (err, result) {
    ...
    new Ctrl(handler).readFromGrid(function (err, data) {
      ...
      setContentType(req, res, data.length, data.filename);
      return res.send(data.fileData);
    });
  });
};
...
exports.zip = function (handler) {
  ...
  rider.list(handler, function (err, result) {
    ...
    var files = [], ctrl = new new Ctrl(handler);
    async.forEach(result.items, function (item, done) {
      ...
      var zipName = handler.params.fileName || "archive.zip"
        , zip     = ph.join(config.app.tmp, zipName);
 ...
 * @param res
 */
exports.qrcode = function (handler) {

  var qrName = helper.uuid()
    , qr     = ph.join(config.app.tmp, qrName);
 ...
 * @param filename
 */
function setContentType(req, res, length, filename) {
  res.setHeader("Content-Type", "application/octet-stream");
...
function canAccess(handler, filename, callback) {
  var roles       = handler.req.session.access
    , curRole     = _.find(roles, function (item) {
        return item[handler.domain];
      })
    , targetRoles = _.filter(curRole[handler.domain].items, function (role) {
