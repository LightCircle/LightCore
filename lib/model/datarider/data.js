...
exports.control = function (handler, collection) {
  ...
  // 普通app(非lightadmin)时需要设置默认code，code没被指定时起作用
  if (process.env.APPNAME && process.env.APPNAME != constant.SYSTEM_DB && !handler.code) {
    handler.code = cache.get('tenant')[0].code;
  }
