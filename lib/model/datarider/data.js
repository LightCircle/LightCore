 ...
 * @module light.lib.model.datarider.data
...
exports.add = function (handler, callback) {
  ...
  // 插入数据
  new Ctrl(handler, api.schema).add(function (err, data) {
    if (err) {
      return callback(err);
    }
...
exports.list = function (handler, callback) {
  ...
  // 检索数据
  new Ctrl(handler, api.schema).list(function (err, data) {
    if (err) {
      return callback(err);
    }
...
exports.get = function (handler, callback) {
  ...
  // 检索数据
  new Ctrl(handler, api.schema).get(function (err, data) {
    if (err) {
      return callback(err);
    }
...
exports.count = function (handler, callback) {
  ...
  new Ctrl(handler, api.schema).total(callback);
};
...
exports.remove = function (handler, callback) {
  ...
  // 删除数据
  new Ctrl(handler, api.schema).remove(callback);
  // sync(handler, api.schema);
};
...
exports.update = function (handler, callback) {
  ...
  // 更新数据
  new Ctrl(handler, api.schema).update(function (err, data) {
    if (err) {
      return callback(err);
    }
...
exports.upsert = function (handler, callback) {
  ...
  // 更新数据
  new Ctrl(handler, api.schema).upsert(callback);
  // sync(handler, api.schema);
};
...
exports.order = function (handler, board) {
  ...
  return sorts.map(sort => {
    ...
    if (sort.dynamic === 'dynamic') {
      const data = handler.params.sort || handler.params.order;
      return [data[sort.key], sort.order.toLowerCase() === 'asc' ? 1 : -1];
    }
    ...
    if (child) {
      ...
      if (_.isArray(val)) { // array 遍历数组，拷贝指定的值
        ...
        _.each(val, function (item, index) {
          ...
          var converted = helper.format(item[child], column.format, timezone);
        ...
        converted = helper.format(val[child], column.format, timezone);
        if (column.alias) {
      ...
      converted = helper.format(data[column.key], column.format, timezone);
...
exports.join = function (handler, data, item, type, callback) {
  ...
  var dataKey = item.key, // 要关联的字段
    link = item.option,   // 被关联的表
    linkKey = item.link,  // 关联字段
    select = item.fields; // 选择项目
  ...
  var selected = _.keys(data.options[link]);
  var keys = mpath.get(dataKey, (type === 4) ? data.items : data);    // 列表类型（type=4）时，在items里取值
  ...
  // 查询
  new Ctrl(handler.copy(filter), link).list(function (err, result) {
...
function sync(handler, schema) {
  ...
  // 表名称是被缓存的表时，进行服务器集群同步
  if (!_.contains(cache, schema)) {
    return;
  }
  ...
  light.model.rider.app.get(handler, {condition: {domain: handler.domain}}, function (err, result) {
    ...
    _.each(result.extend.ap, function (ap) {
      signal.send(ap.name, ap.port, "update.cache", {domain: handler.domain, collection: schema});
    });
  });
}
