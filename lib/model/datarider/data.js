...
exports.add = function (handler, callback) {
  ...
  const api = handler.params.__api;
  ...
  new Ctrl(handler, api.schema).add((err, data) => {
    ...
    // 过滤选择项，并格式化
    if (_.isArray(data)) {
      data = _.map(data, item => exports.reject(item, api, handler.timezone));
    } else {
...
exports.join = function (handler, data, item, type, callback) {
  ...
  // 指定检索条件
  let filter = {condition: {}, select: item.fields};
  ...
  // 查询
  new Ctrl(handler.copy(filter), schema).list(function (err, result) {
    _.each(result.items, function (item) {
      data.options[schema] = data.options[schema] || {};
      data.options[schema][item[item.key] || item._id.toHexString()] = item;
    });
