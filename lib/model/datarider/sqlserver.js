...
function buildScript(board, params) {
  ...
  const selects = _select(board, params);
  const sorts = _sort(board, params);

  // 生成SQL - SELECT
  if (method === 'GET') {
    return selectStatement(params, schema, selects, filter, null, method);
  }

  // 生成SQL - SELECT
  if (method === 'LIST') {
    return selectStatement(params, schema, selects, filter, sorts, method);
  }

  // 生成SQL - COUNT
  if (method === 'COUNT') {
    return selectStatement(params, schema, null, filter, null, method);
  }
  ...
  // 生成SQL - UPDATE
  if (method === 'UPDATE') {
    return updateStatement(params, schema, method);
  }

  // 生成SQL - DELETE
  if (method === 'REMOVE') {
    return deleteStatement(params, schema, method);
  }
...
function _filter(board, params) {
  ...
  if (params.free) {
    return operator.parseFree(params.free);
  }
  ...
  return Object.values(or).map(item => {
    return item.map(and => {
...
function insertStatement(params, schema) {
  ...
  keys.forEach(item => {
    ...
    // 固定项目
    if (['_id', 'createAt', 'createBy', 'updateAt', 'updateBy', 'valid'].includes(item)) {
      column.push(`[${item}]`);
...
function join(handler, data, item, type, callback) {
  ...
  Object.keys(item.conditions).forEach(key => {
    ...
    if (dataKey[0] === '$') {
      let val = _.get((type === 4) ? data.items : data, dataKey.substr(1)); // 列表类型（type=4）时，在items里取值
  ...
  // 查询
  handler.params.script = buildScript(board, filter);
  handler.params.condition = filter.free;

  new ctrl(handler, schema).list((err, result) => {
