...
const _      = require('lodash')
  ...
  , mapping  = require('../../db/mapping')
  , cache    = require('../../cache')
  , helper   = require('../../helper')
;

exports.add = (handler, callback) => {
  handler.params.script = buildScript(handler.api, handler.params, handler.timezone);
  new ctrl(handler, handler.api.schema).add(callback);
};

exports.list = (handler, callback) => {
  handler.params.script = buildScript(handler.api, handler.params, handler.timezone);
  new ctrl(handler, handler.api.schema).list((err, data) => {
    if (err) {
      return callback(err);
    }
...
exports.get = (handler, callback) => {
  handler.params.script = buildScript(handler.api, handler.params, handler.timezone);
  new ctrl(handler, handler.api.schema).get((err, data) => {
    if (err) {
      return callback(err);
    }
...
exports.count = (handler, callback) => {
  handler.params.script = buildScript(handler.api, handler.params, handler.timezone);
  new ctrl(handler, handler.api.schema).count(callback);
};
...
exports.update = (handler, callback) => {
  handler.params.script = buildScript(handler.api, handler.params, handler.timezone);
  new ctrl(handler, handler.api.schema).update(callback);
};

function buildScript(board, params, timezone) {

  // 允许自己写SQL语句，放到script变量中即可
  if (params.script || board.script) {
    return params.script || board.script;
  }
  ...
  const struct = _structure(board.schema)
    , method   = CONST.METHOD_NAME[board.type]
    , map      = new mapping();

  // 根据数据定义，转换参数中的数据类型
  Object.keys(params.condition).forEach(key => {
    Object.assign(params.condition, map.queryParse({[key]: params.condition[key]}, struct.items, {tz: timezone}));
  });

  if ([CONST.METHOD_ADD].includes(method)) {
    params.data = map.setDefaults(params.data, struct.items, {tz: timezone});
    params.data = map.dataParse(params.data, struct.items, {tz: timezone});
  }

  if ([CONST.METHOD_UPDATE].includes(method) && params.id) {
    params.data = map.dataParse(params.data, struct.items, {tz: timezone});
  }

  // 如果指定了id参数，就将id参数转存到condition中
  if ([CONST.METHOD_GET, CONST.METHOD_UPDATE, CONST.METHOD_REMOVE].includes(method) && params.id) {
...
function selectStatement(params, schema, selects, wheres, sorts, method) {
  ...
  let sql = 'SELECT ' + (method === CONST.METHOD_COUNT ? ' COUNT(1) AS COUNT ' : selects.join(', '));
...
function _filter(board, params) {
  ...
  if (params.free) {
    return [operator.parseFree(params.free, board.schema, params)];
  }
  ...
  return _.values(or).map(item => {
    return item
      ...
      .map(and => {
        return operator.getCompiler(and, board.schema, params);
      });
  });
...
function _sort(board, params) {
  ...
  return board.sorts.sort((a, b) => a.index > b.index).map(item => {
...
function join(handler, data, item, type, callback) {
  ...
  const board = cache.get(CONST.SYSTEM_DB_BOARD).find(item => item.schema === schema && item.action === 'list')
    , script  = buildScript(board, filter, handler.timezone);
