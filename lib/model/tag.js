  ...
  async.forEach(tags, function(tag, loop) {
    ...
    rider.tag.get(handler, {condition: {name: tag, type: data.type}}, function(err, result) {
      ...
      if (result && result._id) {
        var updateData = {type: result.type, name: tag, counter: result.counter + 1};

        id.push(result._id);
        rider.tag.update(handler, {data: updateData, id: result._id.toString()}, loop);
      }

      // 新规
      else {
        var addData = {type: data.type, name: tag};
        rider.tag.add(handler, {data: addData}, function (err, result) {
          if (err) {
            return loop(err);
          }
