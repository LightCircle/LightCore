...
function bindApi(handler) {
  ...
  const item = find(rider.board, handler.req.path), define = item.route, params = item.params;
  if (!define) {
  ...
  // 调用用户定义的controller
  let func = controller(define.path, define.class, define.action);
  if (func) {
...
function bindRoute(handler) {
  ...
  log.debug(`route html : url: ${define.url}, template: ${define.template}, _id: ${define._id}`);
  ...
  if (define.class) {
    ...
    if (inject && define.action) {
      ...
      if (func) {
        ...
        log.debug(`inject     : ${define.class}#${define.action}`);
        return func.call(this, handler, (err, result) => {
          if (err) {
            log.error(err, handler.uid);
